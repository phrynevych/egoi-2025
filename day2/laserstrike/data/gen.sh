# Patched version of testdata_tools gen.sh which generates nested directories,
# to enable test case reuse with scoring output validator.
...
group () {
  ...
  mkdir -p "$CURGROUP_DIR"
  groups[$1]=""

  score=$2
  echo "range: 0 $score" > "secret/$1/testdata.yaml"
  echo "on_reject: break
grading: custom
grader_flags: rescale 1000 $score
range: 0 $score" > "$CURGROUP_DIR/testdata.yaml"
...
accept_score: 0
grading: custom
grader_flags: rescale 1000 0" > sample/testdata.yaml
    _update_scores
  fi
}
_setup_dirs
